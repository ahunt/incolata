...
* Power supply: a USB->DC 5525 cable will do (cheaply available on Aliexpress or
  ...
  models from Adafruit are correctly specified).
    * Current/power requirements:
        * 8020A: the manual specifies "5 VDC, 1 amp peak" (or 5W). Most phone
	  chargers will be sufficient. The original power supply on mine is 5V/2.5A
	  (12.5W). The label on the 8020A only states 5V DC.
	* 8020 and 8020M (aka M41 PATS): I was unable to find specifications, the label
	  on the 8020 or 8020M only states 5V DC. 5V/1A is likely to be sufficient
	  given that the hardware is very similar to the 8020A. I have seen at least
	  one 8020 supplied with a 5V/1.5A power supply.
	* 8020Mgen2 (M41A1 PATS): the label on the device states 5V/2A, the original
	  power supply is 5V/3A (15W). Neither the manual nor the specifications
	  specify the device's power requirements. Using a typical 5W phone charger
	  might be a bad idea.
...
> [!WARNING]
> Some TSI power supplies are single-voltage (only 110V, or only 230V). Operator
> beware.
...
> demonstrated by other testers under ambient concentrations close to 10k.
...
> [!Warning]
> Not all modular adapters are suitable - the Delock 66770 looks very similar to
> most other modular adapters, but only contains 4 wires. The Delock 66165 and
> 65462 or equivalent are suitable.

## Disassembly/tools

Tools:

* Internal bolts, and _sometimes_ case bolts: 7/64" allen key (cries in metric).
* Some units have Phillips screws for the case.
...
* [PortaCount Plus Model 8020 - Operation and Service Manual](https://tsi.com/getmedia/9b578bab-ace5-4820-a414-fb0a78712c67/Model_8020_8028_1980092?ext=.pdf):
  describes how to use the device. This is actually for the 8020**A**, but
  should be usable for the 8020 too I think?
* OPERATOR AND UNIT MAINTENANCE MANUAL FOR PROTECTION ASSESSMENT TEST SYSTEM, M41 (NSN: 4240-01-365-8241): 8020**M**gen1 (M41 PATS).
* [PORTACOUNT® PLUS PROTECTION ASSESSMENT TEST INSTRUMENT (PATI) MODEL 8020M -
  Operator's Manual
  ](https://www.kenelec.com.au/wp-content/uploads/2018/09/TSI-8020M-PortaCount-Plus-Protection-Assessment-Test-Instrument-PATI-Manual.pdf):
  8020**M**gen2 (M41A1 PATS).
...
* Replacement Alcohol cartridge:
    * ~~Cartridge & Capsule:
      https://www.harrisonsofhull.com/products/alcohol-cartridge-and-fill-capsule-8022?keyword=8022~~
      No longer available as of September 2025 (link now redirects to 8032 for
      the 803X/804X, which is the wrong size).
...
* Switching valve (in an 8020): MAC 34B-AAA-GFDA-1BA, 3.5V DC, 1.8W. I'm unable
  to find this spec in their current catalogue:
  https://www.macvalves.com/wp-content/uploads/PDFs/Series_Pages/Sm3ways/34catalog.pdf
  (This is not a typo, yes it really is GFDA not GDFA, the latter would be 24V
   DC.)